...
class WordPair {
  ...
  static String _repairMojibake(String value) {
    ...
    for (final encoder in [latin1, const _Windows1250Codec()]) {
      try {
        final decoded = utf8.decode(encoder.encode(directFix));
        final cleaned = _replaceKnownBrokenSequences(decoded);
...
class _Windows1250Codec extends Encoding {
  const _Windows1250Codec();

  @override
  Converter<String, List<int>> get encoder => const _Windows1250Encoder();

  @override
  Converter<List<int>, String> get decoder => throw UnimplementedError();

  @override
  String get name => 'windows-1250-lite';
}

class _Windows1250Encoder extends Converter<String, List<int>> {
  const _Windows1250Encoder();
  ...
  List<int> convert(String input) {
    ...
    for (final rune in input.runes) {
      ...
      final mapped = _unicodeToByte[rune];
      if (mapped != null) {
        out.add(mapped);
      } else if (rune <= 0xFF) {
        out.add(rune);
      } else {
        throw const FormatException('Character not encodable in Windows-1250');
      }
    }
    return out;
  }
